...
Some have used Pubmed to study links between diseases \cite{rajpal2014mining}, which can be thought of as phenotype discovery, to explore relationships between phenotypes and genotypes \cite{pletscher2015diseases}.
Our work focuses on using co-occurrence analysis of Pubmed as a validation tool for annotations.
...
\caption{Feature extraction for phenotypic items \jette{It may be a little confusing to readers that the some terms in the middle column do not appear in the right column. It may be better to scramble the order of the right column and put that in the middle column}}
...
In this example, the term ``heart failure" can also be referred to in various articles using other terms such``myocardial failure" or ``congestive heart disease."
Thus, the appropriate terms must be used so that heart failure as a concept may be discovered in a PubMed article reasonably often when it is being mentioned (recall), but not produce many false positives (precision).
...
In addition, it also provides a tunable knob to find the optimal number of representative n--grams to optimize both precision and recall of the phenotypic item in the articles, which is an concept we will explore in Section~\ref{empirical}.
...
Table \ref{tab:pheno-n-gram}~shows one examples of the original phenotypic item and the ranked list with the eight highest ranked n-grams and their associated relevance scores.
...
Based on our experimental results, we found selecting fifteen or even ten of the top ranked n--grams produced many false positives. 
...
\jette{Note from Dr. Ghosh: with respect to what ground truth? How do we know what is a positive and what is a negative?}
...
Any occurrence of an n-gram that is a part of the set of 3 to 6 n-grams representing a phenotype counted as an appearance for its phenotypic item.\jette{Why is it 3 to 6 n-grams here? I thought it was 6.}
...
In the lift metric, we divide the probability of co-occurrence by the product of each of the probabilities of the individual terms, which normalizes the rate of co-occurrence by the probability of random chance co-occurrence.
...
Due to this dependence on the number of items, we divide each lift by the median of the lifts of that cardinality. 
\jette{I changed this paragraph up a bit and commented out the first sentence of the next paragraph, and I'm not sure I made this clearer, but it might be good to use a term other than ``normalization" here.}
...
\caption{Log lifts versus Phenotype Cardinality \jette{It would be great to go through this figure together. Are you including the lifts of 0 in the average calculation if two phenotypic items did not occur together? I'm also confused when the phenotypic item sets would get so large--are there really phenotypes with 18 terms in them?}}% \jette{Can you change the x axis to be ``Phenotype Cardinality"?}}
\label{fig:log-lift}
\end{figure}
...
\subsection{Results}
\jette{It seems like this section should be reordered. Since \ref{fig:classificationVarNG} was used to figure out how many relevant n-grams should be used, it might be better to start with that figure.}
...
\caption{Classification Scores versus Maximum Number of N-grams Representing Phenotypic items \jette{1) could you change the color of the yellow line to be something darker like red? 2) The current x-axis is a little misleading. It seems like the numbers represent the length of the n-grams rather than the size of the n-gram sets. The x-axis should probably be something like ``Size of Ranked Sßynonym Set"}}
...
\caption{Normalized Average Lift of Curated Phenotypes \jette{Two things: 1) Would it be possible to make this a boxplot of clinically significant groups vs. randomly generated groups? That way we could see the distribution of the standard deviations. 2)  Also, could you change ``clinically significant" to ``Curated Groups?" }}
