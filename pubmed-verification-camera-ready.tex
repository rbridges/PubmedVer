...
We are able to show the method separates randomly generated groups of phenotypes from curated groups to demonstrate known clinical narratives. 
Further, this method has the potential to explain the underlying causes for phenotypes and to identify subsets of phenotype groups that are better predictors for a particular patient cluster.
\end{abstract}
...
For example, two annotators disagreed on the clinical significance of the automatically generated phenotype shown in Table \ref{tab:pheno-example}.
...
While omission of ``unspecified" is a clear step to improve recall without sacrificing precision (``unspecified" is in fact omitted in pre-processing), not every phenotypic item has such a clear solution. \jette{can you show an example where this does not have a clear solution?}
...
In addition, it also provides a tunable knob to find the optimal number of representative n--grams to optimize both precision and recall of the phenotypic item concept in the articles.\jette{Would it be possible for you to back this idea of optimizing with a concrete example or figure?}
...
We define the ``relevance" as the percentage overlap between the sets of PubMed query results from the original phenotypic item and each of its representative n-grams
and calculate it by 1) recording the set of papers returned by each query and 2) finding the size of the intersection between the set of papers returned for the original item and each of the subsequent n--gram queries.
...
\end{tabular}
\end{center}
\caption{Two original phenotypic items and their associated top eight most highly ranked representative n--grams. The score represents the semantic similarity measure derived from the PubMed search index.\jette{I thought it was the size of the overlap between the searches? Is that not right?}}
...
Table \ref{tab:pheno-n-gram}~shows two examples of the original phenotypic item and the ranked list with the eight highest ranked n-grams and their associated relevance scores.
For example, the first synonym `lacrimal apparatus diseases' of the second item `Disorders of lacrimal system' has a relevance of .636, which these phrases appeared together in 63.6\% of Pubmed searches for each phrase separately. \jette{I put this in here and I want to make sure it's right :) }
Based on our experimental results, we found selecting fifteen or even ten of the top ranked n--grams produced many false positives. Using six of the top ranked n-grams gave a tolerable number of false positives. \jette{We should probably show something supporting this, but we can put it in a longer version/journal draft of the paper}
Thus, we represent each phenotypic item with its top six relevant n-grams.
...
In the random subset, we searched for occurrences of the representative/relevant n-grams (generated in the last section) for all items within each phenotype.
...
For every co-occurrence, all possible subsets of co-occurring phenotypes \jette{phenotypic items?} were also counted.
...
While this complete set of co--occurrences is theoretically very large, not every combination of phenotypes is realized in practice.\jette{Does this mean that some searches return a co-occurrence of 0? That might be clearer.}

The lift metric normalizes co--occurrence probability by the probability of all phenotypes co-occurring assuming they are probabilistically independent.\jette{This still isn't super clear to me. Do you mean phenotypic items? Is this the denominator in the lift calculation?}
This means that any lift greater than 1 indicates statistical ``significance".\jette{Why does this mean lift greater than 1 is significant? I'll probably figure it out once I understand the previous sentence.}
...
Due to this dependence on the number of items, the lift of each co-occurrence is normalized according to the number of items it contains.\jette{Is this normalization, or are you are analyzing lift by controlling for cardinality?}
...
Figure \ref{fig:lift-process-cartoon} demonstrates the process of extracting the synonyms for the phenotypic terms, calculating the average standard deviations that a set of phenotypic terms is above the median, and then shows the overall average. \jette{I wanted to double check that the cardinality is correct. It seems like the cardinality should be the number of terms that the blue terms are representing (I'll fix the typo in the last column)}
...
A phenotype is labeled clinically relevant if the average standard deviation of the median is above a threshold.
...
\caption{Log lifts versus Phenotype Cardinality \jette{Can you change the x axis to be ``Phenotype Cardinality"?}}
\label{fig:log-lift}
\end{figure}
...
This way, combinations with larger phenotype cardinality are preferred, but their subsets are not double counted. Then, for each phenotype, the average (normalized) lift in the counted set is calculated. \jette{Would it be possible to condense these two paragraphs into one? It seems like they are saying similar things--are they?}


\begin{figure} [t]
\centering
\includegraphics[width=\linewidth]{kho_josh_all2.png}
\caption{Normalized Average Lift of Marble/Rubik Phenotypes \jette{This is pretty hard to see, could you change the dots to be different shapes? For example, x s, dots, and crosses perhaps?}}
...
\caption{Classification Scores versus Maxium Number of N-grams Representing Phenotypic items \jette{could you change the color of the yellow line to be something darker like red?}}
...
%\end{figure}
First, we look at the normalized lifts generated by the high-throughput methods. 
Figure \ref{fig:kho-josh-all} shows the normalized lift average of the phenotypes generated by Marble and Rubik (\cite {Ho:2014jc,Ho:2014da, wang2015rubik}). 
To do this, we represent each phenotypic term with the top 6 relevant terms. \jette{Could you explain this figure more? Maybe by referring back to Figure 5?}
Figure \ref{fig:classificationVarNG} shows the precision, recall and F1 score for classifying phenotypes to their ``significant" or ``not significant" annotator labels using a lift threshold of 0.028, which was determined to be optimal. \jette{how?}
