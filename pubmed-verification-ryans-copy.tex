...
We note that while lift thresholding classifies phenotypes with relative success in both high-throughput and curated phenotypes, the method does not provide a universal threshold that works well for both. The threshold separating the random and curated phenotypes, 0.014, would be suboptimal for the high--throughput phenotypes. We can infer that phenotypes from different sources need to be grouped so that ideal thresholds can be generated for each source of phenotypes. We also note that many phenotypes are very close to the optimal threshold. This suggests that further work is needed to improve the predictive value of lift thresholding.
